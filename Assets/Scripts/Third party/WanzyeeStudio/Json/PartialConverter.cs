...
namespace WanzyeeStudio.Json
{
    ...
    /// </summary>
    /// <remarks>
    ///     Useful for Unity or 3rd party classes, since we can't insert any
    ///     <c>Newtonsoft.Json.JsonIgnoreAttribute</c>.
    ///     By the way, this works by reflection to access properties.
    ///     Please make sure your property not to be stripped by Unity.
    /// </remarks>
    /// <example>
    ///     It's very easy to make a custom converter, just inherit and override
    ///     <c>GetPropertyNames()</c> as the filter:
    /// </example>
    /// <code>
    ...
    public abstract class PartialConverter<T> : JsonConverter
    {
        ...
        private static void Throw(string name, string format)
        {
            throw new ArgumentException(
                string.Format(format, typeof(T).Name + "." + name), "name");
        }
        ...
        private Dictionary<string, MemberInfo> GetProperties()
        {
            ...
            if (null == _names || !_names.Any())
                throw new InvalidProgramException(
                    "GetPropertyNames() cannot return empty.");

            if (_names.Any(name => string.IsNullOrEmpty(name)))
                throw new InvalidProgramException(
                    "GetPropertyNames() cannot contain empty value.");

            _properties = _names.Distinct()
                .ToDictionary(name => name, name => GetMember(name));
            return _properties;
        }
        ...
        )
        {
            ...
            {
                var _value = _object[_pair.Key]
                    .ToObject(GetValueType(_pair.Value), serializer);
