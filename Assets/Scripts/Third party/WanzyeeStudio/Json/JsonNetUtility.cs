...
namespace WanzyeeStudio.Json
{
    ...
    /// </summary>
    /// <remarks>
    ///     To use Json.NET, please set Unity "PlayerSettings/Api Compatibility Lavel" to .NET
    ///     2.0.
    ///     Then download from its website and import the .NET 2.0 dll.
    ///     Json.NET doesn't support serializing some types originally, e.g.,
    ///     <c>UnityEngine.Vector3</c>.
    ///     This has the <c>defaultSettings</c> includes necessary custom converters by default
    ///     for Unity using it.
    ///     And assign to <c>Newtonsoft.Json.JsonConvert.DefaultSettings</c> when initializing if
    ///     the original <c>null</c>.
    /// </remarks>
    /// <example>
    ...
    public static class JsonNetUtility
    {
        ...
        ///     1. Any custom <c>Newtonsoft.Json.JsonConverter</c> has constructor without
        ///     parameters.
        ...
        /// <summary>
        ///     Initialize when start up, set <c>Newtonsoft.Json.JsonConvert.DefaultSettings</c> if
        ///     not yet.
        /// </summary>
        ...
        private static List<JsonConverter> CreateConverters()
        {
            ...
            var _builtins = new JsonConverter[]
                {new StringEnumConverter(), new VersionConverter()};

            return _customs.Concat(_builtins).Where(converter => null != converter)
                .ToList();
        }
        ...
        private static Type[] FindConverterTypes()
        {
            return AppDomain.CurrentDomain.GetAssemblies(
                ...
                !(null != type.Namespace && type.Namespace.StartsWith("Newtonsoft.Json"))
            ).OrderBy(type =>
                null != type.Namespace && type.Namespace.StartsWith("WanzyeeStudio")
            ).ToArray();
        }
