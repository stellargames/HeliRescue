...
public class TintTextureGenerator : MonoBehaviour
{
    ...
    private Texture2D tintTexture
    {
        get
        {
            ...
            {
                m_TintTexture = new Texture2D(k_TintMapSize, k_TintMapSize,
    ...
    public void Refresh(Grid grid)
    {
        ...
        for (var x = 0; x < w; x++)
        {
            ...
            tintTexture.SetPixel(x, y,
                GetGridInformation(grid).GetPositionProperty(world, "Tint", Color.white));
        }
    ...
    public Color GetColor(Grid grid, Vector3Int position)
    {
        ...
        return GetGridInformation(grid)
            .GetPositionProperty(position, "Tint", Color.white);
    }
    ...
    {
        return new Vector3Int(world.x + tintTexture.width / 2,
            world.y + tintTexture.height / 2, 0);
    }

    private Vector3Int TextureToWorld(Vector3Int texpos)
    {
        return new Vector3Int(texpos.x - tintTexture.width / 2,
            texpos.y - tintTexture.height / 2, 0);
    }
