...
namespace UnityEditor
{
    ...
    internal class IsometricRuleTileEditor : RuleTileEditor
    {
            ...
            RuleTile.TilingRule tilingRule)
        {
            ...
            for (var y = 0; y <= 3; y++)
            {
                var left = rect.xMin + y * rect.width / 6;
                var right = left + rect.width / 2;
                var bottom = rect.yMin + y * rect.height / 6;
                var top = bottom + rect.height / 2;
                Handles.DrawLine(new Vector3(left, top), new Vector3(right, bottom));
            }

            for (var x = 0; x <= 3; x++)
            {
                var left = rect.xMin + x * rect.width / 6;
                var right = left + rect.width / 2;
                var top = rect.yMax - x * rect.height / 6;
