...
namespace UnityEngine
{
    ...
    [Serializable]
    [CreateAssetMenu(fileName = "New Hexagonal Rule Tile",
        menuName = "Tiles/Hexagonal Rule Tile")]
    public class HexagonalRuleTile : RuleTile
    {
        ...
        protected override bool RuleMatches(TilingRule rule,
            ref TileBase[] neighboringTiles, ref Matrix4x4 transform)
        {
            ...
            for (var angle = 0;
                ...
                {
                    transform = Matrix4x4.TRS(Vector3.zero,
            ...
            {
                transform = Matrix4x4.TRS(Vector3.zero, Quaternion.identity,
        ...
        protected override Matrix4x4 ApplyRandomTransform(TilingRule.Transform type,
            Matrix4x4 original, float perlinScale, Vector3Int position)
        {
            ...
            switch (type)
            {
                ...
                case TilingRule.Transform.Rotated:
                    var angle =
                        Mathf.Clamp(Mathf.FloorToInt(perlin * neighborCount), 0,
                            neighborCount - 1) * (360 / neighborCount);
        ...
        protected override void GetMatchingNeighboringTiles(ITilemap tilemap,
            Vector3Int position, ref TileBase[] neighboringTiles)
        {
            if (neighboringTiles != null)
                return;

            if (m_CachedNeighboringTiles == null ||
                m_CachedNeighboringTiles.Length < neighborCount)
        ...
        {
            if (mirrorX && mirrorY) return RotatedOrMirroredIndexes[2, original];
            if (mirrorX) return RotatedOrMirroredIndexes[m_FlatTop ? 3 : 0, original];
            if (mirrorY) return RotatedOrMirroredIndexes[m_FlatTop ? 4 : 1, original];
            return original;
        }
        ...
        private Vector3Int GetOffsetPosition(Vector3Int location, int direction)
        {
            ...
            return m_FlatTop
                ? FlatTopNeighborOffsets[parity, direction]
                : PointedTopNeighborOffsets[parity, direction];
        }
    }
}